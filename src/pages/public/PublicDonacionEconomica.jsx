...
const PublicDonacionEconomica = () => {
  ...
  const handleSubmit = async e => {
    ...
    try {
      const r = await api.post('/publico/donacion/economica', form);
      const donacionId = r.data.donacion?.id || r.data.donacion?._id;
      const pay = await api.post('/publico/donacion/pago', { donacionId });
      window.location.href = pay.data.url;
    } catch (err) { toast.error(err.response?.data?.msg || 'Error al procesar'); setProcessing(false); }
  };
    ...
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'var(--bg-primary)', padding:'40px' }}>
      <div className="glass-card" style={{ width:'100%', maxWidth:'500px', padding:'48px' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:'16px', marginBottom:'32px' }}>
          <div style={{ width:'52px', height:'52px', borderRadius:'12px', background:'rgba(0,212,200,0.1)', border:'0.5px solid rgba(0,212,200,0.2)', display:'flex', alignItems:'center', justifyContent:'center' }}>
            <FaDonate style={{ color:'var(--c-cyan)', fontSize:'22px' }} />
          </div>
          <div>
            <div style={{ fontSize:'11px', color:'var(--c-cyan)', letterSpacing:'3px', marginBottom:'4px' }}>DONACIÓN</div>
            <h1 style={{ fontFamily:"'Playfair Display',serif", fontSize:'26px', color:'var(--text-primary)' }}>Donación económica</h1>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ marginBottom:'20px' }}>
            ...
            <input type="number" name="monto" value={form.monto} onChange={h} placeholder="10.00" min="0.01" step="0.01" className="input-field" required />
          </div>
          ...
          <button type="submit" disabled={processing} className="btn-primary" style={{ width:'100%', padding:'13px' }}>
