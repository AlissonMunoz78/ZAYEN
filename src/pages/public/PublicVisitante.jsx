...
const PublicVisitante = () => {
  ...
  const handleSubmit = async e => {
    ...
    try { await api.post('/publico/visitante', form); setDone(true); toast.success('Visita registrada'); }
    catch (err) { toast.error(err.response?.data?.msg || 'Error al registrar'); }
  ...
  return (
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'var(--bg-primary)', padding:'40px' }}>
      <div className="glass-card" style={{ width:'100%', maxWidth:'460px', padding:'48px' }}>
        {done ? (
          <div style={{ textAlign:'center' }}>
            <FaCheckCircle style={{ fontSize:'64px', color:'var(--c-cyan)', marginBottom:'20px', filter:'drop-shadow(0 0 16px rgba(0,212,200,0.4))' }} />
            <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize:'28px', color:'var(--text-primary)', marginBottom:'12px' }}>¡Bienvenido!</h2>
            <p style={{ color:'var(--text-secondary)', marginBottom:'32px', lineHeight:1.7 }}>Tu visita al Museo Gustavo Orcés ha sido registrada correctamente.</p>
          ...
          <>
            ...
            <div style={{ marginBottom:'36px', marginTop:'20px' }}>
              <div style={{ fontSize:'11px', color:'var(--c-cyan)', letterSpacing:'3px', marginBottom:'12px' }}>REGISTRO PÚBLICO</div>
              <h1 style={{ fontFamily:"'Playfair Display',serif", fontSize:'28px', color:'var(--text-primary)', marginBottom:'8px' }}>Registrar visita</h1>
              <p style={{ color:'var(--text-secondary)', fontSize:'14px' }}>Completa tus datos para registrar tu visita individual.</p>
